...
func createMyAsset(id string) *MyAsset {
	testSpectra := new(Spectrum)
	var testSpectrum []Spectrum

	for i := 0; i < 10; i++ {
		testSpectra.X = float64(i)
		testSpectra.Y = rand.Float64() * float64(i)
		testSpectrum = append(testSpectrum, *testSpectra)
	}

	testMyAsset := new(MyAsset)
	testMyAsset.ID = id
	testMyAsset.DFI = 1.2
	testMyAsset.Pressure = 1.1
	testMyAsset.Temperature = 5.5
	testMyAsset.DTL = 6.666
	testMyAsset.AnalysisTime = 666.666
	testMyAsset.Spectra = testSpectrum
...
func configureStub() (*MockContext, *MockStub) {
	...
	testMyAsset := createMyAsset("config")
	myAssetBytes, _ := json.Marshal(testMyAsset)
	testUnmarshal := new(MyAsset)
	json.Unmarshal(myAssetBytes, testUnmarshal)
...
func TestReadMyAsset(t *testing.T) {
	...
	expectedMyAsset := createMyAsset("config")
...
func TestUpdateMyAsset(t *testing.T) {
	...
	testMyAsset := createMyAsset("updated")
	err = c.UpdateMyAsset(ctx, "statebad", *testMyAsset)
...
func TestQuerryAll(t *testing.T) {
	...
	//_, err = c.QuerryAllAssets(ctx)
	//c.QuerryAllAssets(ctx)
	var expectedMyAssets []*MyAsset
	expectedMyAssets, err = c.QuerryAllAssets(ctx)

	assert.Nil(t, err, "should not return error?")
	assert.Equal(t, []*MyAsset{}, expectedMyAssets)
